...
def check_game_has_finished():
    ...
    if aliens_hit >= alien_count // 2:
        won_message = won_lost_font.render("YOU WON!", True, GREEN)
...
def draw_remaining_aliens():
    ...
    remaining_text = font.render(f'Remaining Aliens: {int(aliens_remaining)}', True, RED)
    screen.blit(remaining_text, (SCREEN_WIDTH - 260, 10))
...
def game_loop():
    ...
    # Flag para rastrear o estado da tecla de disparo
    space_pressed = False
    ...
    while running:
        ...
        if space_pressed and keys[pygame.K_SPACE]:
            if battery.fire():
                # Dispara um foguete
                rocket = create_rocket()
                rockets.append(rocket)
            space_pressed = False  # Reseta o flag para evitar múltiplos disparos contínuos
        if keys[pygame.K_r] and not battery.reloading:
            battery.reload()
        ...
        if battery.reloading:
            reloading_text = font.render('Reloading', True, YELLOW)
            screen.blit(reloading_text, (SCREEN_WIDTH - 150, 40))
...
def setup_difficulty(level):
    ...
    aliens_remaining = alien_count/2
